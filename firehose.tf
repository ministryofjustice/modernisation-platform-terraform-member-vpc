...
locals {
  ...
  # Checks for values of both the endpoint URL and the secret. Only builds the resources if true.
  build_firehose = length(var.kinesis_endpoint_url) > 0 && length(var.kinesis_endpoint_secret_string) > 0
